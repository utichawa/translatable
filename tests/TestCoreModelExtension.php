...
use Utichawa\Translatable\Test\Model;
use Utichawa\Translatable\Test\Model\City;
use Utichawa\Translatable\Test\Model\Company;
use Utichawa\Translatable\Test\Model\Country;
use Utichawa\Translatable\Test\Model\Continent;
use Utichawa\Translatable\Test\Model\Vegetable;
use Utichawa\Translatable\Test\Model\CountryStrict;
use Utichawa\Translatable\Test\Model\CountryGuarded;
use Utichawa\Translatable\Test\Model\CityTranslation;
use Utichawa\Translatable\Test\Model\CountryTranslation;
...
class TestCoreModelExtension extends TestsBase
{
    ...
    public function test_it_returns_false_if_exists_and_dirty_and_parent_save_returns_false()
    {
        ...
        $event->listen('eloquent*', function ($event, $models) use ($that) {
            return get_class(reset($models)) == 'Utichawa\Translatable\Test\Model\Country' ? false : true;
        });
    ...
    public function test_it_returns_false_if_does_not_exist_and_parent_save_returns_false()
    {
        ...
        $event = App::make('events');
        $event->listen('eloquent*', function ($event, $models) use ($that) {
            return get_class(reset($models)) == 'Utichawa\Translatable\Test\Model\Continent' ? false : true;
        });
